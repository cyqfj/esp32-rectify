...
class MicroWebSrv :
    ...
    def __init__( self,
        ...
        #cyq add
        self._tmrweb = Timer(5)
        self._tmrweb.init(period=5, mode=Timer.PERIODIC, callback=self._loop_websvr)

        """
        self._tmrwebsock = Timer(6)
        self._tmrwebsock.init(period=5, mode=Timer.PERIODIC, callback=self._loop_websock)
        """
        self._clients = []
    ...
    #cyq add
    def _loop_websvr(self, t):
        cnt1 = len(self._clients)
        i=0
        while (i<cnt1):
            try:
                self.loopcnt+=1
                #print(self.loopcnt)
                if (self._clients[i]._websock == None):
                    next(self._clients[i].corProcReq)
                else:
                    self._clients[i].corProcWebSock.send(self.AcceptWebSocketCallback)
                i+=1
            except StopIteration:
                #websock初始化返加后，在这里释放corProcReq
                if (self._clients[i].corProcReq != None):
                    del self._clients[i].corProcReq
                    self._clients[i].corProcReq = None
                    #print('free corProcReq', self._clients[i].corProcWebSock)
                else:
                    del self._clients[i].corProcWebSock
                    self._clients[i].corProcWebSock = None
                    if (self._clients[i]._websock._closed):
                        self._clients[i]._closed = True
                        print('ws closed=>main closed')

                #已完成一次页面请求，并关闭了连接
                if (self._clients[i]._closed):
                    del self._clients[i]
                    print('remove one client.')
                    cnt1-=1
                else:
                    if (self._clients[i]._websock == None):
                        self._clients[i].corProcReq = self._clients[i]._processRequest()    #websockt 重新创建协程
                    else:
                        #websockt重新创建WebSock协程
                        self._clients[i].corProcWebSock = self._clients[i]._websock._procWebSock(self.AcceptWebSocketCallback)
                        #执行一次， 避免下次调用send出错, _procWebSock第一条语句是yield
                        next(self._clients[i].corProcWebSock)

                pass

    def _loop_websock(self, t):
        cnt1 = len(self._clients)
        i=0
        while (i<cnt1):
            if (self._clients[i]._websock == None):
    ...
    def _serverProcess(self, sck: socket.socket) :
        #cyq self._started = True
        while True :
            try :
                client, cliAddr = sck.accept()
            except Exception as ex :
                if ex.args and ex.args[0] == 113 :
                    return
                break   #cyq continue
            print('append one client.')
            self._clients.append(self._client(self, client, cliAddr)) #cyq change
            break   #cyq add
        self._started = False
    ...
    def Start(self) :
        if not self._started :
            ...
            #cyq add
            self._server.setsockopt(socket.SOL_SOCKET, 20, self._serverProcess)
            self._started = True
    ...
    class _client :
        ...
        def __init__(self, microWebSrv, sock, addr) :
            sock.settimeout(3)
            ...
            self._websock       = None  #cyq add
            
            if hasattr(sock, 'readline'):   # MicroPython
                self._sockfile = self._sock
            else:   # CPython
                self._sockfile = self._sock.makefile('rwb')

            #cyq add
            self.rcvEvt = False
            sock.setsockopt(socket.SOL_SOCKET, 20, self.recvRequest)
            self.corProcReq =  self._processRequest()
            self._closed = False
            self.corProcWebSock = None
            
            #cyq self._processRequest()

        # ------------------------------------------------------------------------

        def recvRequest(self, sck: socket.socket):
            self.rcvEvt = True
            if (self._websock != None):
                self._websock.rcvEvt = True
        ...
        def _processRequest(self) :
                        ...
                        if not upg :
                            ...
                            if routeHandler :
                             ...
                             and self._microWebSrv.AcceptWebSocketCallback :
                                ...
                                self._websock = MicroWebSocket( sock           = self._sock,
                                                httpClient     = self,
                                                httpResponse   = response,
                                                maxRecvLen     = self._microWebSrv.MaxWebSocketRecvLen,
                                                acceptCallback = self._microWebSrv.AcceptWebSocketCallback )
                                #cyq add 创建协程对象
                                self.corProcWebSock = self._websock._procWebSock(self._microWebSrv.AcceptWebSocketCallback)
                                next(self.corProcWebSock)
                                print('websocket init end')
                                #返回后，定时调度程序将回收corProcReq协程实例
                                return
                        else :
            ...
            try :
                if self._sockfile is not self._sock:
        ...
        def _parseFirstLine(self, response) :
            try :
                elements = self._sockfile.readline().decode().strip().split()
                if len(elements) == 3 :
        ...
        def _parseHeader(self, response) :
            while True :
                elements = self._sockfile.readline().decode().strip().split(':', 1)
                if len(elements) == 2 :
    ...
    class _response :
        ...
        def _write(self, data, strEncoding='ISO-8859-1') :
            if data :
                ...
                while data :
                    n = self._client._sockfile.write(data)
                    if n is None :
                        return False
                    data = data[n:]
                return True
        ...
        def WriteSwitchProto(self, upgrade, headers=None) :
            ...
            self._writeServerHeader()
            self._writeEndHeader()
            if self._client._sockfile is not self._client._sock :
                self._client._sockfile.flush()   # CPython needs flush to continue protocol
