...
class MicroWebSocket :
    ...
    def __init__(self, sock, httpClient, httpResponse, maxRecvLen, acceptCallback) :
        self._sock              = sock
        self._httpCli           = httpClient
        self._closed            = True
        self._lock              = None #cyq allocate_lock()
        self.RecvTextCallback   = None
        self.RecvBinaryCallback = None
        self.ClosedCallback     = None

        if hasattr(sock, 'read'):   # MicroPython
    ...
    # ----------------------------------------------------------------------------
    def _procWebSock(self, acceptCallback):
        yield
        print('-----------first call _procWebSock----------------------')
        self._sock.settimeout(3600)
        self._closed = False
        try :
    ...
    def _receiveFrame(self) :
        try :
            ...
            mask = self._sockfile.read(4) if masked else None
            ...
            if opcode == self._opContFrame or \
               ...
               opcode == self._opBinFrame :
                ...
                if length > 0 :
                    ...
                    if length > len(buf) :
                        return False
                    x = self._sockfile.readinto(buf[0:length])
                    if x != length :
                        return False
                    if masked :
                ...
                if length > len(self._ctrlBuf) :
                    return False
                if length > 0 :
                    x = self._sockfile.readinto(self._ctrlBuf[0:length])
                    if x != length :
                        return False
    ...
    def _sendFrame(self, opcode, data=None, fin=True) :
        if not self._closed and opcode >= 0x00 and opcode <= 0x0F :
            ...
            if dataLen <= 0xFFFF :
                ...
                #cyq self._lock.acquire()
                try :
                    if self._sockfile.write(pack('>BB', b1, b2)) == 2 :
                        if dataLen > 0 :
                            if dataLen >= 0x7E :
                                self._sockfile.write(pack('>H', dataLen))
                            ret = self._sockfile.write(data) == dataLen
                        else :
                            ret = True
                        if self._sockfile is not self._sock :
                            self._sockfile.flush()   # CPython needs flush to continue protocol
                        #cyq self._lock.release()
                        return ret
                except :
                    pass
                #cyq self._lock.release()
